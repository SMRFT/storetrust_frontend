...
const payload = {
  ...
  pending_amount: pendingAmount,
    ...
    const response = await apiRequest(
      `${StoreTrustbaseurl}College-in/update-payment-status/?grn_number=${encodedGrnNumber}`,
      "PATCH",
      payload
    );
...
const formattedPayments = validPayments.map((payment) => {
  ...
  const fetchAllData = useCallback(async (page = 1, pageSize = 100) => {
    ...
    try {
      const response = await apiRequest(
        `${StoreTrustbaseurl}College-in/list/?page=${page}&page_size=${pageSize}`,
        "GET"
      );
      if (!response.success) {
  ...
  const handleEdit = (record) => {
    ...
    // Navigate to CollegeIN page with GRN number as a URL parameter
    navigate(`/CollegeIN/${record.grn_number}`);
  };
  ...
  const handleDelete = async (record) => {
    ...
    try {
      ...
      const response = await apiRequest(
        `${StoreTrustbaseurl}College-in/delete/?grn_number=${encodedGrnNumber}`,
        "PATCH",
        {}
      );
      if (response.success && response.data.status === "success") {
...
export default CollegeGRNReport;