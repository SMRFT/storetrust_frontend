...
const TravellersIN = () => {
  ...
  const fetchVendors = async () => {
    try {
      ...
      const result = await apiRequest(`${StoreTrustbaseurl}vendors/list/`, "GET")
  ...
  useEffect(() => {
    const calculateSummary = () => {
      ...
      const round = (value) => Math.round((Number.parseFloat(value) || 0) * 100) / 100

      // Calculate totals from items
      const totalItemValue = round(items.reduce((sum, item) => sum + (Number.parseFloat(item.itemValue) || 0), 0))
      const totalPurchaseCost = round(items.reduce((sum, item) => sum + (Number.parseFloat(item.purchaseCost) || 0), 0))
      const totalCGST = round(items.reduce((sum, item) => sum + (Number.parseFloat(item.cgstAmt) || 0), 0))
      const totalSGST = round(items.reduce((sum, item) => sum + (Number.parseFloat(item.sgstAmt) || 0), 0))
      const totalDiscount = round(items.reduce((sum, item) => sum + (Number.parseFloat(item.discountedAmt) || 0), 0))
      ...
      const netInvoiceAmount = round(baseNetAmount + round(summary.roundAmount || 0))
    ...
    calculateSummary()
  }, [items, summary.taxOnFreeItems, summary.courierTransportCharge, summary.localTax, summary.roundAmount])
  useEffect(() => {
    if (record) {
      ...
      const fetchRecord = async () => {
        try {
          setLoading(true)
          const result = await apiRequest(`${StoreTrustbaseurl}travellers-in/${grn_number}/`, "GET")
          if (result.success) {
  ...
  const handleInputChange = (e, section = "form") => {
    ...
    if (section === "form") {
      ...
      if (name === "vendor" && value) {
        ...
        if (selectedVendor) {
          setFormData((prev) => ({
            ...
            }, ${selectedVendor.city || ""}, ${selectedVendor.state || ""}, ${selectedVendor.pincode || ""}`.trim(),
      ...
      setSummary((prev) => ({ ...prev, [name]: Number.parseFloat(value) || 0 }))
    }
  }
  ...
  const handleModalInputChange = (e) => {
    ...
    if (name === "name") {
      ...
      if (selectedItem) {
        ...
        setModalForm((prev) => ({ ...prev, name: value, hsn: "" }))
        return
      }
    }
    ...
    setModalForm((prev) => {
      ...
      const packing = Number.parseFloat(updatedForm.packing) || 0
      const noOfUnit = Number.parseFloat(updatedForm.noOfUnit) || 0
      const packingPrice = Number.parseFloat(updatedForm.packingPrice) || 0
      const itemValue = Number.parseFloat(updatedForm.itemValue) || 0
      const cgstPercent = Number.parseFloat(updatedForm.cgstPercent) || 0
      const sgstPercent = Number.parseFloat(updatedForm.sgstPercent) || 0
      const purchaseDiscountPercent = Number.parseFloat(updatedForm.purchaseDiscountPercent) || 0
      const quantity = Number.parseFloat(updatedForm.quantity) || 0
      const free = Number.parseFloat(updatedForm.free) || 0
      ...
      const currentQuantity = Number.parseFloat(updatedForm.quantity) || 0
      const currentFree = Number.parseFloat(updatedForm.free) || 0
      ...
      const currentItemValue = Number.parseFloat(updatedForm.itemValue) || 0
      if (currentItemValue > 0) {
        updatedForm.cgstAmt = ((currentItemValue * cgstPercent) / 100).toFixed(2)
        updatedForm.sgstAmt = ((currentItemValue * sgstPercent) / 100).toFixed(2)
      }

      // 5. Calculate Purchase Cost: Item Value + CGST Amt + SGST Amt
      const cgstAmt = Number.parseFloat(updatedForm.cgstAmt) || 0
      const sgstAmt = Number.parseFloat(updatedForm.sgstAmt) || 0
      let purchaseCost = currentItemValue + cgstAmt + sgstAmt

      // 6. Calculate Discount and adjust Purchase Cost
      let discountedAmt = 0
      const basePurchaseCost = purchaseCost // Store original purchase cost before discount
      ...
      // If discount amount is entered directly
        ...
        const enteredDiscountAmt = Number.parseFloat(value) || 0
        if (enteredDiscountAmt > 0 && basePurchaseCost > 0) {
          // Calculate discount percentage based on original purchase cost
          const calculatedDiscountPercent = (enteredDiscountAmt / basePurchaseCost) * 100
          updatedForm.purchaseDiscountPercent = calculatedDiscountPercent.toFixed(2)
      ...
      // For other field changes, use existing discount values
        ...
        const existingDiscountAmt = Number.parseFloat(updatedForm.discountedAmt) || 0
        const existingDiscountPercent = Number.parseFloat(updatedForm.purchaseDiscountPercent) || 0
        ...
        // If only discount amount exists, maintain it and recalculate percentage
          ...
          const calculatedDiscountPercent = (existingDiscountAmt / basePurchaseCost) * 100
          updatedForm.purchaseDiscountPercent = calculatedDiscountPercent.toFixed(2)
      ...
      const qty = Number.parseFloat(updatedForm.quantity) || 0
      const purchaseCostFinal = Number.parseFloat(updatedForm.purchaseCost) || 0
  ...
  const handleTaxChange = (e) => {
    const taxValue = Number.parseFloat(e.target.value) || 0
    ...
    setModalForm((prev) => {
      ...
      if (itemValue > 0) {
        ...
        const discountPercent = Number.parseFloat(updatedForm.purchaseDiscountPercent) || 0
  ...
  const handleAddItem = () => {
    if (editingItem) {
      setItems((prev) => prev.map((item) => (item.id === editingItem ? { ...modalForm, id: editingItem } : item)))
    } else {
  ...
  const numberToWords = (amount) => {
    ...
    const convertToWords = (num) => {
      ...
      if (num < 100000)
        return (
          convertToWords(Math.floor(num / 1000)) + " Thousand" + (num % 1000 ? " " + convertToWords(num % 1000) : "")
        )
      if (num < 10000000)
        return (
          convertToWords(Math.floor(num / 100000)) + " Lakh" + (num % 100000 ? " " + convertToWords(num % 100000) : "")
        )
      return (
  ...
  const handleConfirmSubmit = async () => {
    try {
      ...
      if (grn_number) {
        ...
        // ✅ Create new record
        result = await apiRequest(`${StoreTrustbaseurl}travellers-in/`, "POST", submitData)
      }

      if (result.success) {
        setGrnData({
          grn_number: result.data?.grn_number || "",
          invoice_no: submitData.invoiceNo,
        })
        setShowGrnDialog(true)

        // Close the invoice preview modal after a short delay
        setTimeout(() => {
  ...
  const handleGrnDialogClose = () => {
    ...
    confirmCancel() // Reset form after dialog closes
  }
  ...
  const VendorDropdown = () => {
    ...
    const handleSelectOption = (vendor) => {
      ...
      const addressParts = [vendor.addressLine1, vendor.addressLine2, vendor.city, vendor.state, vendor.pincode].filter(
        Boolean,
      )
      ...
      <FormGroup>
        ...
        >
          <input
            ...
            placeholder={loadingVendors ? "Loading vendors..." : "Select or type a vendor"}
            style={{
              width: "100%",
  ...
  const handleShowHistory = async (item) => {
    ...
    const itemName = item?.name?.toString().trim() || item?.item_name?.toString().trim()

    console.log("History button clicked - HSN:", hsn, "Item Name:", itemName)

    // Enhanced validation with more specific messages
    if (!itemName || itemName === "") {
      toast.error("Please select an item from the dropdown before viewing history")
      return
    }

    if (!hsn || hsn === "") {
      toast.error("HSN code is required. Please select a valid item with HSN code")
      return
    }
  ...
  const HistoryModal = ({ show, onClose, item, historyData, loading }) => {
    ...
    const getPriceStats = () => {
      ...
      const prices = historyData.map((historyItem) => {
        ...
        return Number.parseFloat(itemData.unitPrice || 0)
      })
    ...
    const calculateTotalStock = () => {
      ...
      return historyData.reduce((total, historyItem) => {
        ...
        const itemTotal = items
          ...
          .reduce((sum, i) => sum + Number.parseInt(i.totalstock || 0), 0)
    ...
    return (
      <HistoryModalOverlay onClick={onClose}>
        <HistoryModalContent onClick={(e) => e.stopPropagation()}>
          <HistoryModalHeader>
            <HistoryModalTitle>
              Purchase History - {item.itemName} (HSN: {item.hsn}) - Total Stock: {totalStock}
              <div style={{ fontSize: "12px", color: "#6b7280", marginTop: "4px" }}>
                Price Range: ₹{priceStats.min.toFixed(2)} - ₹{priceStats.max.toFixed(2)} | Avg: ₹
            ...
            <HistoryTable>
              ...
              <tbody>
                {historyData.map((historyItem, index) => {
                  ...
                  const unitPrice = Number.parseFloat(itemData.unitPrice || 0)
                  ...
                  return (
                    <HistoryTableRow key={index}>
                      <HistoryTableCell>{historyItem.grn_number}</HistoryTableCell>
                      ...
                      <HistoryTableCell>{itemData.name || itemData.item_name || "N/A"}</HistoryTableCell>
                      ...
                      <HistoryTableCell>
                        ₹
                        {Number.parseFloat(itemData.purchaseCost || 0).toLocaleString("en-IN", {
                          minimumFractionDigits: 2,
                        })}
                      </HistoryTableCell>
                      ...
                      <HistoryTableCell>
                        ₹
                        {Number.parseFloat(itemData.mrp || 0).toLocaleString("en-IN", {
                          minimumFractionDigits: 2,
                        })}
                      </HistoryTableCell>
                    </HistoryTableRow>
  ...
  return (
    <Container>
        ...
        <Card>
          ...
          <FormGrid>
            <FormGroup>
              ...
              <Select name="purchaseCategory" value={formData.purchaseCategory} onChange={handleInputChange}>
            ...
            <FormGroup>
              ...
              <Input type="date" name="date" value={formData.date} onChange={handleInputChange} disabled />
            </FormGroup>
            <FormGroup>
              <Label>Vendor ID</Label>
              <Input type="text" name="vendor_id" value={formData.vendor_id} onChange={handleInputChange} />
            </FormGroup>
            <FormGroup>
              <Label>Supplier Address</Label>
              <Input type="text" name="supplierAddress" value={formData.supplierAddress} onChange={handleInputChange} />
            </FormGroup>
          ...
          <TableContainer>
            <StyledTable>
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr>
                    <EmptyRow colSpan="24">No items added yet. Click "Add Item" to get started.</EmptyRow>
                  </tr>
                ) : (
        ...
        <Modal>
          <ModalContent>
            ...
            <ModalBody>
              ...
              <ModalRowGrid>
                ...
                <FormGroup>
                  ...
                  <select name="name" value={modalForm.name} onChange={handleModalInputChange}>
                ...
                <FormGroup>
                  ...
                  >
                    ...
                    <History
                      size={20}
                      style={{
                        cursor: modalForm.name && modalForm.name.trim() !== "" ? "pointer" : "not-allowed",
                        color: modalForm.name && modalForm.name.trim() !== "" ? "#2563eb" : "#9ca3af",
                        transition: "all 0.2s ease",
                        padding: "2px",
                        borderRadius: "3px",
                        opacity: modalForm.name && modalForm.name.trim() !== "" ? 1 : 0.5,
                      }}
                      onClick={(e) => {
                        e.preventDefault()
                        e.stopPropagation()

                        console.log("History icon clicked. Modal form:", modalForm)
                        ...
                        if (!modalForm.hsn || modalForm.hsn.trim() === "") {
                          toast.error("HSN code is missing. Please ensure the selected item has a valid HSN code")
                          return
                        }
                      ...
                      onMouseEnter={(e) => {
                        if (modalForm.name && modalForm.name.trim() !== "") {
                          ...
                          e.currentTarget.style.backgroundColor = "rgba(78, 215, 241, 0.1)"
                        }
                      }}
                      onMouseLeave={(e) => {
                        if (modalForm.name && modalForm.name.trim() !== "") {
                  ...
                  <Input type="text" name="hsn" value={modalForm.hsn} onChange={handleModalInputChange} />
                </FormGroup>
                <FormGroup>
                  <Label>
                    <strong>Batch</strong>
                  </Label>
                  <Input type="text" name="batch" value={modalForm.batch} onChange={handleModalInputChange} />
                </FormGroup>
                <FormGroup>
                  <Label>
                    <strong>Expiry</strong>
                  </Label>
                  <Input type="date" name="expiry" value={modalForm.expiry} onChange={handleModalInputChange} />
                </FormGroup>
              </ModalRowGrid>
              ...
              <ModalRowGrid>
                ...
                <FormGroup>
                  ...
                  <Input type="text" name="packing" value={modalForm.packing} onChange={handleModalInputChange} />
                </FormGroup>
                ...
                <FormGroup>
                  ...
                  <Input type="number" name="noOfUnit" value={modalForm.noOfUnit} onChange={handleModalInputChange} />
                </FormGroup>
                ...
                <FormGroup>
                  ...
                  <Input type="number" name="quantity" value={modalForm.quantity} onChange={handleModalInputChange} />
                </FormGroup>
                ...
                <FormGroup>
                  ...
                  <Input type="number" name="free" value={modalForm.free} onChange={handleModalInputChange} />
                </FormGroup>
              </ModalRowGrid>
              ...
              <TaxRowGrid>
                ...
                <FormGroup>
                  ...
                  <Select name="tax" value={modalForm.tax} onChange={handleModalInputChange}>
              ...
              <ModalRowGrid>
                ...
                <FormGroup>
                  ...
                  <Input type="number" step="0.01" name="mrp" value={modalForm.mrp} onChange={handleModalInputChange} />
                </FormGroup>
              </ModalRowGrid>
            </ModalBody>
            ...
            <ModalFooter>
              ...
              <PrimaryButton onClick={handleAddItem}>{editingItem ? "Update Item" : "Add Item"}</PrimaryButton>
            </ModalFooter>
          </ModalContent>
        </Modal>
      )}
      ...
      {showConfirmDialog && (
        <Modal>
          <SmallModalContent>
            ...
            <ModalFooter>
              <SecondaryButton onClick={() => setShowConfirmDialog(false)}>Keep Editing</SecondaryButton>
          ...
          <InvoiceContent>
            ...
            <InvoiceBody>
              <InvoiceTitle>
                ...
                <div className="address">51/24.Saradha College Road, Salem - 636007,,</div>
                <div className="address">Phone : 04272706666,info@smrft.org</div>
                <div className="address">GST Number :</div>
                <div className="document-title">GOODS RECEIPT NOTE - TRAVELLERS INN</div>
              </InvoiceTitle>
                ...
                <tbody>
                  {items.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      <td className="number-cell">{Number.parseFloat(item.packingPrice || 0).toFixed(2)}</td>
                      <td className="number-cell">{Number.parseFloat(item.purchaseCost || 0).toFixed(2)}</td>
                      <td className="number-cell">{Number.parseFloat(item.mrp || 0).toFixed(2)}</td>
                      <td className="number-cell">{Number.parseFloat(item.discountedAmt || 0).toFixed(2)}</td>
                      <td className="number-cell">{Number.parseFloat(item.itemValue || 0).toFixed(2)}</td>
                      <td className="center-cell">{item.cgstPercent}%</td>
                      <td className="number-cell">{Number.parseFloat(item.cgstAmt || 0).toFixed(4)}</td>
                      <td className="center-cell">{item.sgstPercent}%</td>
                      <td className="number-cell">{Number.parseFloat(item.sgstAmt || 0).toFixed(4)}</td>
                  ...
                  <tr className="total-row">
                    ...
                    <td className="number-cell">{summary.nonTaxableAmount.toFixed(2)}</td>
                    <td></td>
                    ...
                    <td className="number-cell">{summary.totalAmount.toFixed(2)}</td>
                  </tr>
                </tbody>
              </InvoiceTable>
                ...
                <div className="summary-layout">
                  <div className="gst-amounts">
                    <div className="gst-row">
                      ...
                      <span className="amount">: {summary.cgst.toFixed(2)}</span>
                    </div>
                    <div className="gst-row">
                      <span className="label">SGST Amount</span>
                      <span className="amount">: {summary.sgst.toFixed(2)}</span>
                    </div>
                    <div className="gst-row">
                      <span className="label">IGST Amount</span>
                      <span className="amount">: {summary.igst.toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <div className="amounts-table">
                    ...
                    <div className="row">
                      <span>Total GST</span>
                      <span>{(summary.cgst + summary.sgst + summary.igst).toFixed(2)}</span>
                    </div>
                    <div className="row">
              ...
              <InvoiceFooter>
                <div className="footer-item">
                  <div className="label">Verified By: {userId || "N/A"}</div>
                </div>
              </InvoiceFooter>
            </InvoiceBody>

            <InvoiceModalFooter>
              <InvoiceSecondaryButton onClick={() => setShowInvoicePreview(false)}>Cancel</InvoiceSecondaryButton>
              <InvoicePrimaryButton onClick={handleConfirmSubmit}>Confirm & Submit</InvoicePrimaryButton>
            </InvoiceModalFooter>
          </InvoiceContent>
        </InvoiceModal>
      )}
      ...
      />
      {/* Add GRN dialog component in the JSX (add this where your modals are rendered) */}
      {showGrnDialog && (
        <HistoryModalOverlay onClick={handleGrnDialogClose}>
          <HistoryModalContent onClick={(e) => e.stopPropagation()}>
            <HistoryModalHeader>
              <HistoryModalTitle>GRN Generated</HistoryModalTitle>
              <CloseButton onClick={handleGrnDialogClose}>×</CloseButton>
            </HistoryModalHeader>
            <ModalBody style={{ textAlign: "center", padding: "30px 20px" }}>
              <p style={{ fontSize: "16px", marginBottom: "15px", color: "#333" }}>
                GRN No <strong>{grnData.grn_number}</strong> for Invoice No <strong>{grnData.invoice_no}</strong>
              </p>
            </ModalBody>
            <ModalFooter>
              <SuccessButton onClick={handleGrnDialogClose}>OK</SuccessButton>
            </ModalFooter>
          </HistoryModalContent>
        </HistoryModalOverlay>
      )}
