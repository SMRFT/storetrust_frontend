...
import { FaEye, FaEyeSlash, FaTimes, FaPrint, FaFileExcel } from "react-icons/fa"
...
const getIntentStatus = (items) => {
  ...
  if (statuses.every((s) => s === "Partially Approved")) return "Partially Approved"
...
function TravellersIntentReport() {
    ...
    try {
      ...
      const updatedItems = await Promise.all(
        items.map(async (item) => {
          ...
          return { ...item, hsn, totalStock: freshStock, dynamicTotalStock: freshStock }
        }),
      )

      // Update the intent with fresh stock data
      const updatedIntents = intents.map((i) => (i.intent_number === intentNumber ? { ...i, items: updatedItems } : i))
  ...
  const handleSaveRow = async (intentId, itemIndex) => {
    ...
    const updatedItem = {
      ...item,
      approved:
        editValues.status === "Approved"
          ? requestedQuantity
          : editValues.status === "Partially Approved"
            ? approvedQty
            : 0,
  ...
  const updateStatusAndStock = async (intentId, itemIndex, newStatus, approvedQuantity, updatedItem = null) => {
    ...
    let updatedIntents = intents.map((i) => (i.intent_number === intentId ? { ...i, items } : i))
    ...
    if (
      ...
      const restoreSuccess = await restoreStockInDB(item.itemName, item.hsn || "", prevApprovedQty, employeeId)
  ...
  const updateDynamicStock = (intents) => {
    const sortedIntents = [...intents].sort((a, b) => new Date(a.date) - new Date(b.date))
    const stockMap = {}
  ...
  const restoreStockOnRejection = (intentsList, intentId, itemName, restoredQty) => {
    ...
    const sortedIntents = [...intentsList].sort((a, b) => new Date(a.date) - new Date(b.date))
  ...
  const handleModalSave = async (approvedQty) => {
    ...
    if (intent) {
      ...
      if (item) {
        const hsn = item.hsn || (await fetchHsnByItemName(item.itemName))
        const freshStock = await getTotalStock(item.itemName, hsn)
        console.log(`[v0] Fresh stock fetched for ${item.itemName}: ${freshStock}`)
      ...
      if (items.length > 0) {
        items.forEach((item, idx) => {
          const totalStockDisplay =
            item.dynamicTotalStock !== undefined ? item.dynamicTotalStock : (item.totalStock ?? 0)
      ...
      if (items.length > 0) {
        items.forEach((item, iidx) => {
          ...
          exportData.push({
            ...
            "Total Stock": item.dynamicTotalStock !== undefined ? item.dynamicTotalStock : (item.totalStock ?? 0),
      ...
      {expandedIntent && (
        <Overlay>
          <PopupContainer>
            ...
            >
              {/* Title */}
              <h3 style={{ margin: 0, color: "#662549" }}>Items for {expandedIntent}</h3>
                    ...
                    items.map((item, itemIndex) => {
                      ...
                      return (
                        <tr key={item.item_id || itemIndex}>
                            ...
                            {isEditing ? (
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  else if (newStatus === "Pending" || newStatus === "Reject") newApproved = "0"
                              ...
                              >
                                ...
                                <option value="Approve" disabled={currentStock <= 0 || requestedQty > currentStock}>
                                  Approve
                                </option>
                                <option value="Reject">Reject</option>
                              </select>
                          ...
                          <SubTd>
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) => setEditValues((prev) => ({ ...prev, approved: e.target.value }))}
                                disabled={editValues.status !== "Partially Approve"}
                                min={0}
                                max={Number(item.quantity)}
                                style={{
                                  backgroundColor: editValues.status === "Partially Approve" ? "white" : "#eee",
                                }}
                              />
                          ...
                          <SubTd>
                            {item.status && item.status !== "Pending" ? item.approved_by || "Pending" : "Pending"}
                          </SubTd>
                          <SubTd>
